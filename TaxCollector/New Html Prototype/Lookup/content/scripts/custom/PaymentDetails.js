...
addCreditCardDetails:function(){
	...
	var cc = {
								...
								"ExpDate":  $("#txtExpireDate").val().replace('/','|'),
								"CVV":  $("#txtCCV").val(),
...
populateCreditCardDetails:function(){
	...
	$("#txtExpireDate").val(dbObject.BillingDetails.ExpDate.replace('_','/').replace('|','/'));
...
populateSubtotal: function(){
	...
	$("#paysubTotalAmount").html(addThousandsSeparator(total));
},
populateOrderDetails: function(){
	debugger;
          ...
          html += '<div id="pnlItem1">';
           for (var i = 0; i < dbObject.Products.length; i++) {
          		...
          		html += '<td class="col-sm-6 text-uppercase"><label id="ItemName">'+(row.ProductName.toUpperCase().indexOf('TEST')>=0?row.ProductName.replace(' ',' &').replace('_',''):row.ProductName)+' </label> </td>';
           		html += '<td style="text-align:right"><label id="ItemAmount">$' + addThousandsSeparator(parseFloat(bill2payAccountDetails.getItemAmount(i)).toFixed(2))+'</label> </td>';
          		html +=  '</tr>';
          		html +=  '<tr>';
          		if(row.ACC1!=null){
        ...
        html +=  '<td style="text-align:right"><label id="subTotalAmount"  >'+addThousandsSeparator(bill2payProducts.getCartTotalAmount().toString())+'</label> </td>';
        html += '</tr>';                         
        html +=  '</div>';
        html +=  '</table>';
        html += '</div>';
